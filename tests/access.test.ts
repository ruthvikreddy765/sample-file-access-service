...
async function registerSample(owner: string, files: { name: string }[] = []) {
  const res = await request(app)
    .post('/samples')
    .send({ owner, files });
  expect(res.status).toBe(201);
  return res.body.sample as { id: string; fileIds: string[]; files: { id: string }[] };
}

async function grantAccess(sampleId: string, userId: string) {
  const res = await request(app)
...
async function setQc(fileId: string, status: 'passed' | 'failed') {
  const res = await request(app)
...
describe('POST /samples', () => {
  it('registers a sample with no files', async () => {
    const res = await request(app).post('/samples').send({ owner: 'alice' });
    expect(res.status).toBe(201);
  ...
  it('owner automatically has access to the sample', async () => {
    ...
    const res = await request(app).get(`/files/${fileId}/download?userId=alice`);
    expect(res.status).toBe(200);
  });
});
...
describe('PATCH /files/:fileId/qc', () => {
  it('updates a file from pending to passed', async () => {
    const sample = await registerSample('alice', [{ name: 'data.csv' }]);
    const fileId = sample.files[0].id;
    const res = await request(app).patch(`/files/${fileId}/qc`).send({ status: 'passed' });
    expect(res.status).toBe(200);
  ...
  it('updates a file from pending to failed', async () => {
    const sample = await registerSample('alice', [{ name: 'data.csv' }]);
    const fileId = sample.files[0].id;
    const res = await request(app).patch(`/files/${fileId}/qc`).send({ status: 'failed' });
    expect(res.status).toBe(200);
  ...
  it('returns 400 for invalid status value', async () => {
    const sample = await registerSample('alice', [{ name: 'data.csv' }]);
    const fileId = sample.files[0].id;
    const res = await request(app).patch(`/files/${fileId}/qc`).send({ status: 'unknown' });
    expect(res.status).toBe(400);
  });

  it('returns 404 for a non-existent file', async () => {
    const res = await request(app).patch('/files/no-such-file/qc').send({ status: 'passed' });
    expect(res.status).toBe(404);
  });
});
...
describe('GET /files/:fileId/download', () => {
  it('returns a download URL when user has access and QC passed', async () => {
    ...
    await setQc(fileId, 'passed');
    const res = await request(app).get(`/files/${fileId}/download?userId=bob`);
    expect(res.status).toBe(200);
  ...
  it('returns 404 when file does not exist', async () => {
    const res = await request(app).get('/files/ghost-file/download?userId=alice');
    expect(res.status).toBe(404);
  });
  ...
  it('denies access even if QC passed but user was never granted access', async () => {
    ...
    const fileId = sample.files[0].id;
    await setQc(fileId, 'passed');
    const res = await request(app).get(`/files/${fileId}/download?userId=carol`);
    expect(res.status).toBe(403);
  });
});